...
exports.SpermVideosAnalyze = (req, res) => {
  ...
  uploadedFile.forEach((file) => {
    ...
    if (supportedVideoFormats.includes(fileFormat)) {
      ...
      return res
        .status(400)
        .send(`${file.name}: 지원하지 않는 비디오 포맷입니다.`);
    }
  });
  ...
  exec(
    `python3 ../python/list_patient.py ${directoryPath}`,
    (error, stdout) => {
      if (error) {
      ...
      res.status(200).json({
        data: spermCounts,
        per: { chromosome: chromosomePer, intfertility: intfertilityPer },
      });
    },
  );
};
...
exports.getChromosome = (req, res) => {
  exec(
    'python3 ../tracking_video/module_test/module_4.py 2>&1 | tee -a ../tracking_video/module_test/module4.log',
    (error, stdout) => {
      if (error) {
        console.error(`Error: ${error.message}`);
...
exports.getInfertility = (req, res) => {
  exec(
    'python3 ../tracking_video/module_test/module_5.py 2>&1 | tee -a ../tracking_video/module_test/module5.log',
    (error, stdout) => {
      if (error) {
        console.error(`Error: ${error.message}`);
